...
pub struct YELTableBody;

pub enum YELTableBodyMsg {}

#[derive(Clone, PartialEq, Properties)]
pub struct YELTableBodyProps {
    #[prop_or_default]
...
impl Component for YELTableBody {
    type Message = YELTableBodyMsg;
    type Properties = YELTableBodyProps;
        ...
        html! {
            ...
            <colgroup>
                {ctx.props().columns.clone().into_iter().map(|_|{
                        ...
                        let g = h;
                        h = h + 1;
                        html!(<col name={format!("el-table_1_column_{}", b)} width={format!("{}", width_vec[g])}/>)
                    }).collect::<Html>()}
            </colgroup>
            <tbody>
